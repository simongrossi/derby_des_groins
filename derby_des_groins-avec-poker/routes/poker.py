...
def _award_pot(table: PokerTable, winners: list[PokerPlayer], all_players: list[PokerPlayer],
    ...
    IMPORTANT : on ne crédite le vrai compte BitGroins qu'à la FIN de la partie
    (ou lors d'un cashout volontaire). En cours de jeu, les chips sont de la monnaie
    interne à la table — seul le champ `chips` du joueur est mis à jour.
    """
    if not winners:
        return
    ...
    winner_names = []
    for w in winners:
        w.chips = round((w.chips or 0) + share, 2)
        u = User.query.get(w.user_id)
        winner_names.append(u.username if u else '?')

    # Construire le mapping hand_label pour les logs finaux
    hand_label_map = {}
    if hand_results:
        for pid, rank in hand_results.items():
            hand_label_map[str(pid)] = hand_name(rank)

    # Enregistrer l'historique (sans toucher aux vrais comptes)
    ...
    # Joueurs tombés à 0 → spectateurs (APRÈS avoir crédité les gains aux gagnants)
    for p in all_players:
        if not p.is_spectator and p.chips <= 0:
            p.is_spectator = True
            p.status = 'spectator'
    ...
    # Vérifier si la partie est terminée (≤ 1 joueur avec des jetons)
    still_playing = [p for p in all_players if not p.is_spectator and p.chips > 0]
    if len(still_playing) <= 1:
        table.status = 'finished'
        # Créditer TOUS les joueurs ayant des jetons (gagnant(s) final/finaux)
        for p in all_players:
            if (p.chips or 0) > 0:
                hand_info = hand_label_map.get(str(p.id), '')
                label_detail = f' ({hand_info})' if hand_info else ''
                credit_user_balance(
                    p.user_id, p.chips,
                    reason_code='poker_win',
                    reason_label='Gains Groin Poker',
                    details=f'Gains finaux {p.chips:.0f} BG{label_detail} — table #{table.id}',
                    reference_type='poker_table', reference_id=table.id,
                )
                p.chips = 0
        return
