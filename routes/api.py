...
def api_race_replay(race_id):
    ...
    if race.status not in ('finished', 'cancelled'):
        return jsonify({'error': 'La course est pas encore terminée, patience mon grand !'}), 425
    if not race.replay_json:
        return jsonify({'error': 'Pas de replay disponible, le greffier a oublié de filmer'}), 404

    import json as _json
    replay = _json.loads(race.replay_json)

    # Attach participant metadata (emoji, owner) from DB
    participants_db = Participant.query.filter_by(race_id=race_id).all()
    participant_meta = {p.id: {'emoji': p.emoji, 'owner': p.owner_name, 'finish_position': p.finish_position} for p in participants_db}
...
def api_latest_race_replay():
    """Retourne le replay de la dernière course terminée."""
    race = Race.query.filter_by(status='finished').order_by(Race.finished_at.desc()).first()
    if not race:
        return jsonify({'error': 'Aucune course terminée, les cochons sont encore en pyjama'}), 404
    return api_race_replay(race.id)


@api_bp.route('/live')
def race_live():
    """Page d'animation live de la dernière course."""
    user = None
