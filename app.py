...
def create_app():
    ...
    def _render_error_template(template_name, status_code):
        try:
            ...
            return (
                f"Erreur {status_code}. Le serveur rencontre un probleme temporaire.",
                status_code,
    ...
    def log_request(response):
        ignored_prefixes = (
            '/static', '/health', '/admin/auth-logs', '/api/notifications/poll', '/api/race/live-state'
        )
        if request.path.startswith(ignored_prefixes) or request.path.startswith('/chat'):
            return response
        
        # Ne pas logger les GET pour alléger SQLite si on veut être prudent
        ...
        try:
            ...
            logger.exception("Impossible d'ecrire le log d'action")
        return response
    ...
    with app.app_context():
        try:
            ...
            logger.exception("Erreur critique lors de l'initialisation DB")
...
def run_seeders(with_admin=True):
    if with_admin: ensure_admin_user()
    seed_users()
...
def register_cli_commands(app):
    ...
    def purge_auth_logs_command(days):
        ...
        click.echo(f'🧹 Auth logs purgés: {deleted_count}')
...
def seed_users():
    ...
    for u in default_users:
        ...
        if existing:
            ...
            if u.get('admin'): existing.is_admin = True
        else:
...
def ensure_balance_transaction_snapshots():
    logged_user_ids = {uid for (uid,) in db.session.query(BalanceTransaction.user_id).distinct().all()}
    for user in User.query.all():
        if user.id in logged_user_ids: continue
        record_balance_transaction(
            user_id=user.id, amount=round(float(user.balance or 0.0), 2),
            balance_before=0.0, balance_after=round(float(user.balance or 0.0), 2),
            reason_code='snapshot', reason_label='Ouverture du journal BitGroin',
...
def seed_game_data():
    ...
    if not CerealItem.query.first():
        for i, (key, c) in enumerate(CEREALS.items()):
            item = CerealItem(key=key, name=c['name'], emoji=c['emoji'], cost=c['cost'], hunger_restore=c.get('hunger_restore', 0), energy_restore=c.get('energy_restore', 0), weight_delta=c.get('weight_delta', 0), valeur_fourragere=c.get('valeur_fourragere', 100), sort_order=i)
            for stat in ('vitesse', 'endurance', 'agilite', 'force', 'intelligence', 'moral'): setattr(item, f'stat_{stat}', c.get('stats', {}).get(stat, 0.0))
            db.session.add(item)
    if not TrainingItem.query.first():
        for i, (key, t) in enumerate(TRAININGS.items()):
            item = TrainingItem(key=key, name=t['name'], emoji=t['emoji'], energy_cost=t.get('energy_cost', 0), hunger_cost=t.get('hunger_cost', 0), weight_delta=t.get('weight_delta', 0), min_happiness=t.get('min_happiness', 0), happiness_bonus=t.get('happiness_bonus', 0), sort_order=i)
            for stat in ('vitesse', 'endurance', 'agilite', 'force', 'intelligence', 'moral'): setattr(item, f'stat_{stat}', t.get('stats', {}).get(stat, 0.0))
            db.session.add(item)
    if not SchoolLessonItem.query.first():
        for i, (key, l) in enumerate(SCHOOL_LESSONS.items()):
            item = SchoolLessonItem(key=key, name=l['name'], emoji=l['emoji'], question=l['question'], answers_json=_json.dumps(l['answers'], ensure_ascii=False), xp=l.get('xp', 20), wrong_xp=l.get('wrong_xp', 5), energy_cost=l.get('energy_cost', 10), hunger_cost=l.get('hunger_cost', 4), min_happiness=l.get('min_happiness', 15), happiness_bonus=l.get('happiness_bonus', 5), wrong_happiness_penalty=l.get('wrong_happiness_penalty', 5), sort_order=i)
            for stat in ('vitesse', 'endurance', 'agilite', 'force', 'intelligence', 'moral'): setattr(item, f'stat_{stat}', l.get('stats', {}).get(stat, 0.0))
            db.session.add(item)
    if not HangmanWordItem.query.first():
        for i, word in enumerate(COCHON_PENDU_WORDS): db.session.add(HangmanWordItem(word=word, sort_order=i))
    db.session.commit()

def _init_grain_market():
    from data import BOURSE_DEFAULT_POS
    if not GrainMarket.query.first():
        db.session.add(GrainMarket(id=1, cursor_x=BOURSE_DEFAULT_POS, cursor_y=BOURSE_DEFAULT_POS))
        db.session.commit()

app = create_app()

if __name__ == '__main__':
    app.run(debug=os.environ.get('FLASK_DEBUG', 'false').lower() == 'true', host='0.0.0.0', port=int(os.environ.get('PORT', 5001)))
